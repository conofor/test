		...
		$transformValue = function ($val, string $type, string $q = "'") use (&$transformValue) {
			if (is_array($val)) { # Массивы  ни к чему не приводятся кроме как к строке
				if (in_array($type, ['#', 'a'])) { # Вероятен идентификатор как массив
					foreach ($val as $k => $v) {
						# (ключ = значение) только для ?a - спецификатора, значение форматируется в зависимости от его типа
						$val[$k] = ((is_numeric($k) || $type == '#') ? '' : "`$k` = ") . $transformValue($v, '', $q); # (идентично универсальному параметру без спецификатора)
					}
					return implode(', ', $val);
				}
